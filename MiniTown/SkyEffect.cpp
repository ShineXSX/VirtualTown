...
bool SkyEffect::InitAll(ID3D11Device * device)
{
	...
	if (!Render::IsInit())
		throw std::exception("Render need to be initialized first!");
...
void SkyEffect::SetRenderDefault(ID3D11DeviceContext * deviceContext)
{
	...
	deviceContext->RSSetState(Render::RSNoCull.Get());

	deviceContext->PSSetSamplers(0, 1, Render::SSLinearWrap.GetAddressOf());
	deviceContext->OMSetDepthStencilState(Render::DSSLessEqual.Get(), 0);
