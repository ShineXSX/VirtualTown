...
HRESULT SkyBox::InitSkyBox(ID3D11Device* device, ID3D11DeviceContext* deviceContext, const std::wstring& cubemapFilename, float skySphereRadius, bool generateMips)
{
	// 防止重复初始化造成内存泄漏
...
ID3D11ShaderResourceView* SkyBox::GetTextureCube()
{
	return m_pTextureCubeSRV.Get();
}

void SkyBox::Draw(ID3D11DeviceContext* deviceContext, SkyEffect& skyEffect, const Camera& camera)
{
