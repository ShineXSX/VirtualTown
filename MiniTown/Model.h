...
struct ModelProperty
{
	...
	ModelProperty() : material(), texDiffuse(), vertexBuffer(), indexBuffer(),
		vertexCount(), indexCount(), indexFormat() {}

	ModelProperty(const ModelProperty&) = default;
	ModelProperty& operator=(const ModelProperty&) = default;

	ModelProperty(ModelProperty&&) = default;
	ModelProperty& operator=(ModelProperty&&) = default;
...
struct Model
{
	...
	Model();
	Model(ID3D11Device * device, const ObjLoader& model);
	//设置缓冲区
	...
	//放置模型
	void SetModel(ID3D11Device * device, const ObjLoader& model);
	...
	std::vector<ModelProperty> modelParts;
