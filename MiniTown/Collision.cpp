...
Ray Ray::ScreenToRay(const Camera & camera, float screenX, float screenY)
{
	...
	// 从NDC坐标系变换回世界坐标系
	XMMATRIX Transformation = XMMatrixMultiply(camera.GetViewXM(), camera.GetProjXM());
	Transformation = XMMatrixInverse(nullptr, Transformation);

	XMVECTOR Target = XMVectorMultiplyAdd(V, Scale, Offset);
	Target = XMVector3TransformCoord(Target, Transformation);
...
std::vector<Transformation> XM_CALLCONV Collision::FrustumCulling(
	const std::vector<Transformation>& transforms, const DirectX::BoundingBox& localBox, DirectX::FXMMATRIX View, DirectX::CXMMATRIX Proj)
{
	std::vector<Transformation> acceptedData;
...
std::vector<Transformation> XM_CALLCONV Collision::FrustumCulling2(
	const std::vector<Transformation>& transforms, const DirectX::BoundingBox& localBox, DirectX::FXMMATRIX View, DirectX::CXMMATRIX Proj)
{
	std::vector<Transformation> acceptedData;
...
std::vector<Transformation> XM_CALLCONV Collision::FrustumCulling3(
	const std::vector<Transformation>& transforms, const DirectX::BoundingBox& localBox, DirectX::FXMMATRIX View, DirectX::CXMMATRIX Proj)
{
	std::vector<Transformation> acceptedData;
